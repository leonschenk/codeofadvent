...
int main(void){
    ...
    snailfishnumber numbers[lines];

    for (int i = 0; i < lines && infile >> numbers[i]; i++) {
    }
    
    long maxResult = 0;
    for (int i = 0; i < lines; i++) {
        for (int j = 0; j < lines; j++) {
            snailfishnumber addition;
            addition += numbers[i];
            addition += numbers[j];
            long res = addition.magnitude();
            if (res > maxResult) {
                maxResult = res;
            }
        }
    ...
    std::cout << "Het resultaat is: " << maxResult << std::endl;
}

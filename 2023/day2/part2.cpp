...
int main(int argc, char* argv[]) {
    ...
    for (auto& g : games) {
        Subset maxSet;
        for (auto& s : g.hands) {
            maxSet.red = std::max(s.red, maxSet.red);
            maxSet.green = std::max(s.green, maxSet.green);
            maxSet.blue = std::max(s.blue, maxSet.blue);
        }
        count += (maxSet.red * maxSet.green * maxSet.blue);
    }
